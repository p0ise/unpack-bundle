...
- Optionally use the automated release script locally:
  ...
  Or use the GitHub Actions **Release** workflow once the repository is on GitHub.
  The workflow will update the changelog, create a GitHub Release, and publish
  the package to npm (when `NPM_TOKEN` is configured as a repository secret).

  See `CONTRIBUTING.en.md` for details on the release workflow and changelog generation.
